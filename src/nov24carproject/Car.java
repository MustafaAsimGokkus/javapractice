...
/*
 ...
 Q1) Can we create instance variable in abstract class? Yes
 ...
 Q5) Constructors are to create objects. If we cannot create object
     from abstract class then what is the usage of 
     
     i) to reach the instance variables of abstract class from concrete child classes
     ii)to assign values to instance variables using super constructor call
     with parameter when we create an object from concrete child class
 Static block : 
 i)it runs only once when the class is 
     loaded into memory
 ii)to initialize static variables we use it    
